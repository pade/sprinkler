...
class Relay(BaseGpio):
    ...
    def write(self, pchannel, pvalue):
        if pvalue:
            self.bus.write_byte_data(self.__config['default']['DEVICE_ADDR'], pchannel+1, 0xFF)
        else:
            self.bus.write_byte_data(self.__config['default']['DEVICE_ADDR'], pchannel+1, 0x00)

    def read(self,pchannel):
        return self.bus.read_byte_data(self.__config['default']['DEVICE_ADDR'], pchannel+1)
